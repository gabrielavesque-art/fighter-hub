...
async function fromUFC(name){
  ...
  // les découpes officielles vivent dans /styles/athlete_bio_*/
  const m = html.match(/https?:\/\/[^"'\s)]*\/styles\/athlete_bio[^"'\s)]*\.(?:png|jpg|jpeg|webp)/i);
  if(m) return { src: m[0].replace(/&amp;/g,'&'), source:'ufc' };

  // repli : image sociale de la page athlète
  const og = html.match(/<meta[^>]+property=["']og:image["'][^>]+content=["']([^"']+)["']/i);
  if(og && /athlete|fighter/i.test(og[1])) return { src: og[1].replace(/&amp;/g,'&'), source:'ufc-og' };
